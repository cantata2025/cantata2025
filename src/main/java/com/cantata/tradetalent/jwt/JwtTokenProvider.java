package com.cantata.tradetalent.jwt;
...
public class JwtTokenProvider {
    ...
    @PostConstruct
    public void init(){
        // Base64 로 인코딩된 키를 디코딘 후 hmac-sha 알고리즘으로 다시 암호화
    ...
    // 토큰 발급 로직
    // 엑세스 토근 생성 (사용자가 들고다닐 신분증) : 유효기간이 짧음
    public String createAccessToken(String username){
        return createToken(username, jwtProperties.getAccessTokenValidityTime());

    }
    // 리프레시 토큰 생성 (서버가 보관할 신분증을 재발급하기 위한 정보) : 유효기간이 비교적 김
    ...
    private String createToken(String username, long validityTime){
        ...
        return Jwts.builder()
                ...
                .setSubject(username)   // 이 토큰을 구별할 유니크한 값
                .signWith(key)  // 서명
                .compact()
                ;
    }
